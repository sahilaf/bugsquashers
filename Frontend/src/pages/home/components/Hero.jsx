    ...
    <div className=" h-screen pt-32">
      ...
      <div className="order-2 lg:order-1 h-full w-full lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left justify-start px-4 sm:px-8 md:px-16 lg:pl-32 pt-5 md:pt-16  overflow-hidden">
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-4 sm:gap-6 w-full sm:w-auto">
          <button className="w-full sm:w-auto px-6 py-3 bg-primary text-white rounded-full text-sm sm:text-base font-semibold hover:bg-primary-dark transition-colors duration-300">
            GET STARTED
          </button>
          <button className="w-full sm:w-auto px-6 py-3 bg-action_red text-white rounded-full text-sm sm:text-base font-semibold hover:bg-action_red-dark transition-colors duration-300">
            MARKETPLACE
          </button>
        </div>

        {/* Search Bar */}
        <div className="mt-6 flex items-center bg-light_grey rounded-full pl-6 pr-3 w-full sm:w-[440px] h-12 sm:h-14">
          <input
            type="text"
          ...
          <button className="bg-white h-8 sm:h-10 w-10 sm:w-12 rounded-full flex items-center justify-center hover:bg-gray-100 transition-colors duration-300">
            <img src={camera} alt="Camera" className="h-5 w-5 cursor-pointer" />
          </button>
        </div>
      </div>
      </div>
      ...
      <div className="bg-transparent px-4 sm:px-8 md:px-16 lg:px-32 xl:px-32 w-full">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {cardsData.map((card, index) => (
            <div
              key={card.id}
              className={`${colorPalette[index % colorPalette.length]} rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow duration-300`}
            >
