...
export const CartProvider = ({ children }) => {
  ...
  const fetchCart = useCallback(async () => {
    ...
    try {
      ...
      const message = err.response?.status === 404 ? 'Cart not found' : 'Could not load cart';
      setError(message);
      setCartItems([]);
      toast.error(message);
    } finally {
  ...
  const removeFromCart = useCallback(async (productId) => {
    ...
    try {
      const res = await axios.delete(`http://localhost:3000/api/cart/${uid}/item/${productId}`);
  ...
  const sendUpdateRequest = async (uid, productId, quantity) => {
    return axios.put(`http://localhost:3000/api/cart/${uid}/item/${productId}`, {
      quantity,
    });
  };
  ...
  const analyzeError = (err) => ({
    isHtmlResponse: typeof err.response?.data === 'string' && 
                   err.response.data.includes('<!DOCTYPE html>'),
    needsRetry: err.response?.status === 404 && 
  ...
  const cartTotal = cartItems?.reduce((total, item) => 
    total + ((item?.productId?.price || 0) * (item?.quantity || 0)), 0) || 0;
